...
One of the other challenges I came across was properly scaling my webpage. The different SVG elements did not scale with the page, so they would overlap and disort the visualizations. Eventually I fixed this problem by implementing bootstrap in my HTML code. The whole page in one row, which is split in two collumns. These collumns are again split in rows, and more collumns. This made the whole page scalable. It took me a long time to figure out the proper way to use Bootstrap, which delayed the process a lot. Decreasing the size of the page works fine now, but increasing the size still does not work properly. The visualisations scale according to their size, and the collumns and rows rearrange themselves when they fall of the page. Only the tables start overlapping, and I cannot fix the problem. 
